...
const Profile = () => {
    ...
    useEffect(() => {
        ...
        async function fetchRecipes() {
            ...
            const recipes = await getFavorites(recipeIds).catch(err => console.log('Error', err));
    ...
    return (
        <div>
            ...
            {recipeIds.length> 0 && state.favoriteRecipes.length===0 ? "Oops something went wrong" : 
            recipeIds.length>0 ? state.favoriteRecipes.map((recipe) => (
